...
var s_account="tiadsldevicesprod"
...
s.linkInternalFilters="javascript:,www.aliceadsl.telecomitalia.it"
...
function s_doPlugins(s)
{
  ...
  /* -- Set Page View Event - Mirror Page View ----------------*/
  s.events=s.apl(s.events,'event1',',',2)
  /*-----------------------------------------------------------*/
  
  
  /* -- Copying props into eVars ------------------------------*/
  ...
  if(!s.hier1) 
  {
    if(s.channel) s.hier1=s.apl(s.hier1,s.channel,',',2);
    for(var i=1;i<5;i++)
    {
      if(s['prop'+i]) s.hier1=s.apl(s.hier1,s['prop'+i],',',2);
    }
  }
...
s.trackingServer="metrics.tim.it"
s.trackingServerSecure="ssl-metrics.tim.it"
