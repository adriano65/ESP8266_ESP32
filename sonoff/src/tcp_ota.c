...
LOCAL void ICACHE_FLASH_ATTR ota_rx_cb(void *arg, char *data, uint16_t len) {
    ...
    ip_addr_t addr;
    IP4_ADDR(&addr, conn->proto.tcp->remote_ip[0], conn->proto.tcp->remote_ip[1], conn->proto.tcp->remote_ip[2], conn->proto.tcp->remote_ip[3]);
    if (ota_ip == 0) {
            ...
            case RECEIVING_FIRMWARE: {
                ...
                if ((ota_firmware_len == SPI_FLASH_SEC_SIZE) || (ota_firmware_received == ota_firmware_size)) {
                    ...
                    if (ota_firmware_received <= SPI_FLASH_SEC_SIZE) {
                        DBG("This is the first block, check the header");
                    ...
                    if (current == UPGRADE_FW_BIN1) {
                        ...
                        address = 4*1024 + ESP_FLASH_MAX + 16*1024 + 4*1024;
                    } else {
                    ...
                    //DBG("Flashing address %05x, total received = %d", address, ota_firmware_received);
                    SpiFlashOpResult res = spi_flash_write(address, (uint32 *)ota_firmware, SPI_FLASH_SEC_SIZE);
                    ota_firmware_len = 0;
...
void ICACHE_FLASH_ATTR ota_init() {
    ...
    espconn_tcp_set_max_con_allow(&ota_conn, 1);
    espconn_regist_time(&ota_conn, 15, 0);
}
