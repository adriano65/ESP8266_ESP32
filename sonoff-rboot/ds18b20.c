...
void write_bit(int bit) {
  ...
  if (bit) {
	...
	set_gpio_mode(sensor_data.pin, GPIO_INPUT, GPIO_FLOAT);
	os_delay_us(55);
	}
  else{
	//55us low
...
int reset(void) {
	...
	set_gpio_mode(sensor_data.pin, GPIO_OUTPUT, GPIO_FLOAT);
	gpio_write(sensor_data.pin, 0);
    os_delay_us(500);
...
bool DS18B20read() {
    ...
    //os_delay_us(750);
	sleepms(800);				// 12 bit conversion time (750 ms)
    if (reset()) return false;
    write_byte(DS1820_SKIP_ROM);		// skip ROM command
