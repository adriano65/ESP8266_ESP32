...
FlashConfig flashDefault = {
  ...
  .rssi_enable = 0,
  .mqtt_enable = 0, 
  .mqtt_status_enable = 0,
  .mqtt_timeout = 5, 
  .mqtt_clean_session = 1,
  .mqtt_port    = 5800, 
  .mqtt_keepalive = 60,
  .mqtt_host    = "\0", 
  .mqtt_clientid = "sunmi\0",
  .mqtt_username= "\0", 
  .mqtt_password = "\0", 
  .IOPort_bit0=0,
  .IOPort_bit1=0,
...
static int ICACHE_FLASH_ATTR selectPrimary(FlashFull *ff0, FlashFull *ff1) {
  ...
  DBG("FLASH chk=0x%04x crc=0x%04x full_sz=%d sz=%d chip_sz=%d\n", crc16_data((unsigned char*)ff0, sizeof(FlashFull), 0), crc, sizeof(FlashFull), sizeof(FlashConfig), getFlashSize());
