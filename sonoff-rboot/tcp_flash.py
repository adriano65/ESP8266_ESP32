...
if len(sys.argv) < 3:
	print 'Usage: '
	print '   Usage:'
	print '     tcp_flash.py <host|IP> <rom0.bin> <rom1.bin>'
	print ''
	print '   Where:'
	print '     <host|IP>    the hostname or IP address of the ESP8266 to be flashed.'
	print '     <rom0.bin>  the file holding the first flash format file.'
	print '                  Used when the currently used flash is rom1.bin'
	print '     <rom1.bin>  the file holding the second flash format file.'
	print '                  Used when the currently used flash is rom0.bin'
	sys.exit(1)
...
f = None
response = s.recv(128)
if response == "rom0.bin\r\n":
	print 'Flashing \"{}\"...'.format(user1bin)
	f = open(user1bin, "rb")
elif response == "rom1.bin\r\n":
