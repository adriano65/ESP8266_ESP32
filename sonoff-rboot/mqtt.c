...
static void ICACHE_FLASH_ATTR mqtt_tcpclient_recv(void* arg, char* pdata, unsigned short len) {
  ...
  do {
    ...
    if (pMQTTclient->connState != MQTT_CONNECTED) {
      ...
      DBG("ERROR: recv in invalid state %d\n", pMQTTclient->connState);
      mqtt_doAbort(pMQTTclient);
      return;
    }
    ...
    DBG("Recv type=%s id=%04X len=%d; Pend type=%s id=%02X\n",
    ...
    case MQTT_MSG_TYPE_CONNACK:
      ...
      pMQTTclient->reconTimeout = 1; // reset the reconnect backoff
      break;
    ...
    case MQTT_MSG_TYPE_PUBLISH: { // incoming publish
        ...
        //DBG("Recv PUBLISH qos=%d\n", msg_qos, mqtt_get_publish_topic(pMQTTclient->in_buffer));
  ...
  switch (pMQTTclient->connState) {
	case MQTT_CONNECTED:
		if (pMQTTclient->pending_buffer != NULL && --pMQTTclient->timeoutTick == 0) {
		  DBG("looks like we're not getting a response in time, abort the connection\n");
		  mqtt_doAbort(pMQTTclient);
		...
		if (pMQTTclient->keepAliveAckTick > 0 && --pMQTTclient->keepAliveAckTick == 0) {
		...
		if (pMQTTclient->keepAliveTick > 0 && --pMQTTclient->keepAliveTick == 0) {
...
static void ICACHE_FLASH_ATTR mqtt_send_message(MQTT_Client* pMQTTclient) {
  ...
  if (buf == NULL || pMQTTclient->sending) return; // ahem...
  ...
  if (msg_type == MQTT_MSG_TYPE_PINGREQ) {
	DBG("%s: msg_type == MQTT_MSG_TYPE_PINGREQ\n", __FUNCTION__);
...
bool ICACHE_FLASH_ATTR MQTT_Publish(MQTT_Client* pMQTTclient, const char* topic, const char* data, uint16_t data_length, uint8_t qos, uint8_t retain) {
  ...
  if (buf == NULL) {
    os_printf("%s: Cannot allocate buffer for %d byte publish\n", __FUNCTION__, buf_len);
    return FALSE;
	}
  ...
  if (!mqtt_msg_publish(&msg, topic, data, data_length, qos, retain, &msg_id)){
    os_printf("%s: Queuing Publish failed\n", __FUNCTION__);
    os_free(buf);
    return FALSE;
	}
  ...
  DBG("Publish, topic: \"%s\", data: %s\n", topic, data);
  pMQTTclient->msgQueue = PktBuf_Push(pMQTTclient->msgQueue, buf);

  if (!pMQTTclient->sending && pMQTTclient->pending_buffer == NULL) {
    mqtt_send_message(pMQTTclient);
	}
  else   DBG("mqtt_send_message fail\n");
...
* @retval None
*/
void ICACHE_FLASH_ATTR MQTT_Init(MQTT_Client* pMQTTclient, char* host, uint32 port, uint8_t security, uint8_t sendTimeout, char* client_id, char* client_user, char* client_pass, uint8_t keepAliveTime) {
  ...
  pMQTTclient->connect_info.username = (char*)os_zalloc(os_strlen(client_user) + 1);
  os_strcpy(pMQTTclient->connect_info.username, client_user);

  pMQTTclient->connect_info.password = (char*)os_zalloc(os_strlen(client_pass) + 1);
  os_strcpy(pMQTTclient->connect_info.password, client_pass);
...
void ICACHE_FLASH_ATTR MQTT_Connect(MQTT_Client* pMQTTclient) {
  ...
  DBG("to %s:%d %p\n", pMQTTclient->host, pMQTTclient->port, pMQTTclient->pCon);
  if (UTILS_StrToIP((const char *)pMQTTclient->host, (void*)&pMQTTclient->pCon->proto.tcp->remote_ip)) {
    int8_t err;
    if (pMQTTclient->security)
      err = espconn_secure_connect(pMQTTclient->pCon);
    else
      err = espconn_connect(pMQTTclient->pCon);
    if (err != 0) {
      os_printf("MQTT_Connect: Failed to connect %d\n", err);
...
static void ICACHE_FLASH_ATTR mqtt_doAbort(MQTT_Client* pMQTTclient) {
  DBG("%s: Disconnecting from %s:%d (%p)\n", __FUNCTION__, pMQTTclient->host, pMQTTclient->port, pMQTTclient->pCon);
...
void ICACHE_FLASH_ATTR MQTT_Disconnect(MQTT_Client* pMQTTclient) {
  ...
  mqtt_doAbort(pMQTTclient);
  //void *out_buffer = pMQTTclient->mqtt_connection.buffer;
...
void ICACHE_FLASH_ATTR MQTT_Free(MQTT_Client* pMQTTclient) {
  ...
  if (pMQTTclient->connect_info.username) os_free(pMQTTclient->connect_info.username);
  if (pMQTTclient->connect_info.password) os_free(pMQTTclient->connect_info.password);
  os_memset(&pMQTTclient->connect_info, 0, sizeof(mqtt_connect_info_t));
