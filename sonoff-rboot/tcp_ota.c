        ...
        switch (ota_state) {
            ...
            case RECEIVING_HEADER: {
                ...
                if (eol > 0) {
                    ...
                    if (!strncmp("GetNextFlash", ota_buffer, eol - 2)) {
					  ...
					  DBG("receiving HEADER, current ROM%d\n", unit);
					  if (unit == ROM0) {
						espconn_send(conn, "rom1.bin\r\n", 10);
						} 
					  else {
						espconn_send(conn, "rom0.bin\r\n", 10);
						}
					  } 
            ...
            case RECEIVING_FIRMWARE: {
                ...
                if ((ota_firmware_len == SPI_FLASH_SEC_SIZE) || (ota_firmware_received == ota_firmware_size)) {
                    ...
                    if (current == ROM0) {
                        // The next flash, rom1.bin, will start after 4KB boot, user1, 16KB user params, 4KB reserved.
                    ...
                    if (ota_firmware_received == ota_firmware_size) {
					  DBG("We've flashed all of the firmware now, reboot into the new firmware.\n");
					  ...
					  os_timer_arm(&ota_reboot_timer, 4000, 1);
					  }
                }
                break;
            }
